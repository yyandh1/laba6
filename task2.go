package main

import (
	"fmt"
	"math/rand"
	"time"
)

func generateMasterKey(MASTER_KEY *[]byte, mt *rand.Rand) {
	letters := func() int { return 'a' + mt.Intn('z'-'a') }
	numbers := func() int { return '0' + mt.Intn(10) }
	for i := 1; i <= 8; i++ {
		*MASTER_KEY = append(*MASTER_KEY, byte(letters()))
		*MASTER_KEY = append(*MASTER_KEY, byte(numbers()))
	}
}

func shiftRows(line *[]byte) {
	v := make([]byte, len(*line))
	for i := 1; i < len(*line); i++ {
		v[i-1] = (*line)[i]
	}
	v[len(*line)-1] = (*line)[0]
	*line = v
}

func invShiftRows(line *[]byte) {
	v := make([]byte, len(*line))
	for i := 0; i < len(*line); i++ {
		v[i] = (*line)[(i+(i%4)*(i%4))%len(*line)]
	}
	*line = v
}

func subBytes(line *[]byte) {
	Sbox := []byte{
		,
	}

	v := make([]byte, len(*line))
	for _, i := range *line {
		v = append(v, Sbox[i])
	}
	*line = v
}

func invSubBytes(line *[]byte) {
	invSboxData := []byte{
		,
	}

	v := make([]byte, len(*line))
	for _, i := range *line {
		v = append(v, invSboxData[i])
	}
	*line = v
}

func addRoundKey(FIRST, SECOND *[]byte) []byte {
	v := make([]byte, 4)
	for i := 0; i < 4; i++ {
		v[i] = (*FIRST)[i] ^ (*SECOND)[i]
	}
	return v
}

func keyExpansion(MASTER_KEY *[]byte, ROUND_KEYS *[][]byte) {
	Rcon := []byte{
		0x00, 0x00, 0x00, 0x00,
		0x01, 0x00, 0x00, 0x00,
		0x02, 0x00, 0x00, 0x00,
		0x04, 0x00, 0x00, 0x00,
		0x08, 0x00, 0x00, 0x00,
		0x10, 0x00, 0x00, 0x00,
		0x20, 0x00, 0x00, 0x00,
		0x40, 0x00, 0x00, 0x00,
		0x80, 0x00, 0x00, 0x00,
		0x1b, 0x00, 0x00, 0x00,
		0x36, 0x00, 0x00, 0x00,
	}

	v := make([]byte, 4)
	i := 0
	Nk := 4
	Nb := 4
	Nr := 10

	*ROUND_KEYS = make([][]byte, Nb*(Nr+1))
	for i := range *ROUND_KEYS {
		(*ROUND_KEYS)[i] = make([]byte, 4)
	}

	for i < Nk {
		v = []byte{(*MASTER_KEY)[4*i], (*MASTER_KEY)[4*i+1], (*MASTER_KEY)[4*i+2], (*MASTER_KEY)[4*i+3]}
		(*ROUND_KEYS)[i] = v
		i++
	}

	i = Nk
	for i < (Nb * (Nr + 1)) {
		v = (*ROUND_KEYS)[i-1]
		if i%Nk == 4 {
			shiftRows(&v)
			subBytes(&v)

			for k := 0; k < len(v); k++ {
				v[k] = v[k] ^ Rcon[i/Nk]
			}
		} else if Nk > 6 && i%Nk == 4 {
			subBytes(&v)
		}

		for j := 0; j < len(v); j++ {
			(*ROUND_KEYS)[i][j] = (*ROUND_KEYS)[i-Nk][j] ^ v[j]
		}
		i++
	}
}

func galoisMultiply(a, b byte) byte {
	result := byte(0)
	var carry byte

	for i := 0; i < 8; i++ {
		if b&1 == 1 {
			result ^= a
		}

		carry = a & 0x80
		a <<= 1

		if carry == 0x80 {
			a ^= 0x1b
		}

		b >>= 1
	}

	return result
}

func generateMultBy2() []byte {
	multBy2 := make([]byte, 256)
	for i := 0; i < 256; i++ {
		multBy2[i] = galoisMultiply(byte(i), 2)
	}
	return multBy2
}

func generateMultBy3() []byte {
	multBy3 := make([]byte, 256)
	for i := 0; i < 256; i++ {
		multBy3[i] = galoisMultiply(byte(i), 3)
	}
	return multBy3
}

func generateMultBy14() []byte {
	multBy14 := make([]byte, 256)
	for i := 0; i < 256; i++ {
		multBy14[i] = galoisMultiply(byte(i), 14)
	}
	return multBy14
}

func generateMultBy9() []byte {
	multBy9 := make([]byte, 256)
	for i := 0; i < 256; i++ {
		multBy9[i] = galoisMultiply(byte(i), 9)
	}
	return multBy9
}

func generateMultBy13() []byte {
	multBy13 := make([]byte, 256)
	for i := 0; i < 256; i++ {
		multBy13[i] = galoisMultiply(byte(i), 13)
	}
	return multBy13
}

func generateMultBy11() []byte {
	multBy11 := make([]byte, 256)
	for i := 0; i < 256; i++ {
		multBy11[i] = galoisMultiply(byte(i), 11)
	}
	return multBy11
}

func mixColumn(line *[]byte) {
	multBy2 := generateMultBy2()
	multBy3 := generateMultBy3()

	v := make([]byte, 4)
	v[0] = multBy2[(*line)[0]] ^ multBy3[(*line)[1]] ^ (*line)[2] ^ (*line)[3]
	v[1] = multBy2[(*line)[1]] ^ multBy3[(*line)[2]] ^ (*line)[0] ^ (*line)[3]
	v[2] = multBy2[(*line)[2]] ^ multBy3[(*line)[3]] ^ (*line)[0] ^ (*line)[1]
	v[3] = multBy2[(*line)[3]] ^ multBy3[(*line)[0]] ^ (*line)[1] ^ (*line)[2]
	*line = v
}

func invMixColumns(line *[]byte) {
	multBy14 := generateMultBy14()
	multBy9 := generateMultBy9()
	multBy13 := generateMultBy13()
	multBy11 := generateMultBy11()

	v := make([]byte, 4)
	v[0] = multBy14[(*line)[0]] ^ multBy9[(*line)[1]] ^ multBy13[(*line)[2]] ^ multBy11[(*line)[3]]
	v[1] = multBy14[(*line)[1]] ^ multBy9[(*line)[2]] ^ multBy13[(*line)[3]] ^ multBy11[(*line)[0]]
	v[2] = multBy14[(*line)[2]] ^ multBy9[(*line)[3]] ^ multBy13[(*line)[0]] ^ multBy11[(*line)[1]]
	v[3] = multBy14[(*line)[3]] ^ multBy9[(*line)[0]] ^ multBy13[(*line)[1]] ^ multBy11[(*line)[2]]
	*line = v
}

func blockGenerate(text string, BLOCK *[][][]byte) {
	for len(text)%16 != 0 {
		text += " "
	}

	v := make([][][]byte, 0)
	sixteen := make([][]byte, 4)
	for i := 0; i < 4; i++ {
		sixteen[i] = make([]byte, 4)
	}

	for i := 0; i < len(text); i++ {
		a := (i % 16) % 4
		b := (i % 16) / 4
		sixteen[a][b] = text[i]

		if (i+1)%16 == 0 {
			v = append(v, sixteen)
			sixteen = make([][]byte, 4)
			for i := 0; i < 4; i++ {
				sixteen[i] = make([]byte, 4)
			}
		}
	}
	*BLOCK = v
}

func CIPHER(BLOCK, ROUND_KEYS [][]byte, MASTER_KEY []byte) [][]byte {
	v := make([][]byte, 4)
	for i := 0; i < 4; i++ {
		v[i] = addRoundKey(BLOCK[i], ROUND_KEYS[i])
	}

	for i := 1; i <= 9; i++ {
		for j := 0; j <= 3; j++ {
			subBytes(&v[j])
			shiftRows(&v[j])
			mixColumn(&v[j])
			v[j] = addRoundKey(v[j], ROUND_KEYS[i])
		}
	}

	for j := 0; j <= 3; j++ {
		subBytes(&v[j])
		shiftRows(&v[j])
		v[j] = addRoundKey(v[j], ROUND_KEYS[10])
	}

	return v
}

func DECIPHER(BLOCK, ROUND_KEYS [][]byte, MASTER_KEY []byte) [][]byte {
	roundKeys := make([][]byte, 44)
	keyExpansion(&MASTER_KEY, &roundKeys)

	v := make([][]byte, 4)
	for i := 0; i < 4; i++ {
		v[i] = addRoundKey(BLOCK[i], ROUND_KEYS[10])
	}

	for i := 9; i >= 1; i-- {
		for j := 0; j <= 3; j++ {
			v[j] = addRoundKey(v[j], ROUND_KEYS[i])
			invMixColumns(&v[j])
			invShiftRows(&v[j])
			invSubBytes(&v[j])
		}
	}

	for j := 0; j <= 3; j++ {
		v[j] = addRoundKey(v[j], ROUND_KEYS[0])
	}

	for j := 0; j <= 3; j++ {
		invShiftRows(&v[j])
		invSubBytes(&v[j])
	}

	return v
}

func main() {
	rand.Seed(time.Now().UnixNano())

	text := ""
	fmt.Print("Input text for cipher >> ")
	fmt.Scanln(&text)

	var BLOCK [][][]byte
	blockGenerate(text, &BLOCK)
	fmt.Println("----------------------------------------------")
	fmt.Println("The encryption block:")
	for _, i := range BLOCK {
		for _, j := range i {
			for _, k := range j {
				fmt.Printf("%4d ", k)
			}
			fmt.Println()
		}
	}
	fmt.Println("\n----------------------------------------------")

	var MASTER_KEY []byte
	generateMasterKey(&MASTER_KEY, rand.New(rand.NewSource(time.Now().UnixNano())))
	fmt.Print("128-bit master key: ")
	for _, i := range MASTER_KEY {
		fmt.Printf("%d", i)
	}
	fmt.Println("\n----------------------------------------------")

	var ROUND_KEYS [][]byte
	keyExpansion(&MASTER_KEY, &ROUND_KEYS)
	fmt.Println("Generated keys:")
	for _, i := range ROUND_KEYS {
		for _, j := range i {
			fmt.Printf("%4x ", j)
		}
		fmt.Println()
	}
	fmt.Println("\n----------------------------------------------")

	PREV := make([]byte, 16)
	DEFOLT := make([]byte, 16)
	TEK := make([][]byte, 4)
	ECRYPT := make([][][]byte, 0)

	for _, B := range BLOCK {
		res := make([][]byte, 4)
		TEK = CIPHER(PREV, ROUND_KEYS, MASTER_KEY)

		for i := 0; i < 4; i++ {
			res[i] = make([]byte, 4)
			for j := 0; j < 4; j++ {
				res[i][j] = TEK[i][j] ^ B[i][j]
			}
		}
		ECRYPT = append(ECRYPT, res)

		PREV = make([]byte, 16)
		for i := 0; i < 4; i++ {
			for j := 0; j < 4; j++ {
				PREV[i*4+j] = TEK[j][i]
			}
		}
	}

	fmt.Println("The final cipher after encryption:")
	for _, t := range ECRYPT {
		for _, i := range t {
			for _, j := range i {
				fmt.Printf("%4d ", j)
			}
			fmt.Println()
		}
	}

	fmt.Println("\n----------------------------------------------")

	PREV = DEFOLT
	DECRYPT := make([][][]byte, 0)

	for _, B := range ECRYPT {
		res := make([][]byte, 4)
		TEK = DECIPHER(B, ROUND_KEYS, MASTER_KEY)

		for i := 0; i < 4; i++ {
			res[i] = make([]byte, 4)
			for j := 0; j < 4; j++ {
				res[i][j] = TEK[i][j]
			}
		}
		DECRYPT = append(DECRYPT, res)

		PREV = make([]byte, 16)
		for i := 0; i < 4; i++ {
			for j := 0; j < 4; j++ {
				PREV[i*4+j] = TEK[j][i]
			}
		}
	}

	DECRYPT_1 := make([][][]byte, 0)

	for _, B := range ECRYPT {
		res := make([][]byte, 4)
		TEK = DECIPHER(B, ROUND_KEYS, MASTER_KEY)

		for i := 0; i < 4; i++ {
			res[i] = make([]byte, 4)
			for j := 0; j < 4; j++ {
				res[i][j] = TEK[i][j]
			}
		}
		DECRYPT_1 = append(DECRYPT_1, res)
	}

	fmt.Println("Message after decryption:")
	for _, decrypt := range DECRYPT {
		for j := 0; j < len(decrypt[0]); j++ {
			for i := 0; i < len(decrypt); i++ {
				fmt.Printf("%c", decrypt[i][j])
			}
		}
	}
	fmt.Println("\n----------------------------------------------")
}


